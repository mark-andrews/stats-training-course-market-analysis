...
first_pass_instructions <- readLines("instructions2.md") |> str_c(collapse = "\n")
results <- process_posts(tess_posts, instructions = first_pass_instructions, llm = "Llama", database='tess', backup_file = "tess_results_backup.Rds")
tess_posts2 <- tess_posts[map_lgl(results, ~str_detect(., '^Yes.*'))]

instructions <- readLines("instructions.md") |> str_c(collapse = "\n")
results2 <- process_posts(tess_posts2, instructions = instructions, llm = "Llama", database='tess', backup_file = "tess_results_backup.Rds")


# convert results to a data-frame
# tricky because of the values that are lists
results_df <- postprocess_results(results, tess_posts) |>
  discard(is.null) |>
...
saveRDS(list(raw = results, data_frame = results_df), "data/tess_training_course_posts_llama_results.Rds")
